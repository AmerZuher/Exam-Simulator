...
(function () {
  ...
  /* ---------------- question pools ----------------
     Pool definitions, cloning, shuffling and grading now live in
     App.examShared so Practice Mode can share the exact same rules. */
  const POOLS = App.examShared.POOLS;
  const poolCount = App.examShared.poolCount;
  const cloneQuestion = App.examShared.cloneQuestion;
  const shuffleQuestionOptions = App.examShared.shuffleQuestionOptions;
  const resolveTimeLimit = App.examShared.resolveTimeLimit;
  const medianOf = App.examShared.medianOf;
  ...
  function updateSide() {
    ...
    const cells = session.questions.map(function (q, i) {
      ...
      const isAns = App.examShared.isAnswered(q, resp);
      if (isAns) answered++;
      return (
  ...
  View.finish = function () {
    ...
    const graded = qs.map(function (q, i) {
      ...
      const isCorrect = grade.isCorrect, isSkipped = grade.isSkipped;
      perType[q.type].t++;
