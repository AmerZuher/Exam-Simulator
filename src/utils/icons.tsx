// SVG Icon Registry — ported 1:1 from the original vanilla app's icons.js
// so every icon renders pixel-identical to the source design.
const ICONS = {
  // A mastery ring (the same progress-ring motif used across Dashboard,
  // Study, Review and Results) with a gap "completed" by the checkmark —
  // reads as "you finished the ring" rather than a generic check-in-circle.
  logo: '<circle cx="12" cy="12" r="8.2" stroke-width="1.5" stroke-linecap="round" stroke-dasharray="37 14.5" transform="rotate(-90 12 12)"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.8" d="M7.4 12.6l3.3 3.3 6.1-7.2"/>',
  ...
  user: '<circle cx="12" cy="8.2" r="4"/><path stroke-linecap="round" stroke-linejoin="round" d="M4.5 20.5v-1a6 6 0 016-6h3a6 6 0 016 6v1"/>',
  ...
  logout: '<path stroke-linecap="round" stroke-linejoin="round" d="M9 21H5a2 2 0 01-2-2V5a2 2 0 012-2h4"/><path stroke-linecap="round" stroke-linejoin="round" d="M16 17l5-5-5-5"/><path stroke-linecap="round" d="M21 12H9"/>',
};
...
export const BANK_ICONS: IconName[] = [
...
// A trimmed, curated subset shown in the icon pickers (bank card / group
// card) — the full BANK_ICONS list stays intact for the deterministic
// badge-fallback hash in BankBadge, which must not change.
export const PICKER_ICONS: IconName[] = [
  'grad', 'book', 'cards', 'brain', 'target',
  'flask', 'code', 'globe', 'briefcase', 'calculator',
  'gear', 'shield',
];

export const BANK_TONES = ['acc', 'teal', 'ok', 'warn', 'bad'] as const;
